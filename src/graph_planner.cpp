...
void setInitialPose(DMap &stMap,
    ...
    for (int layer_idx = start_layer; layer_idx < end_layer;++layer_idx) {
        ...
        auto result = handler.computeSplines(path, start_heading, goal_heading); // coeffs_x, coeffs_y, kappa, el_lengths, cost
        if (!result) {
            cerr << "computeSplines failed for edge: " << endl;
        continue;
    }

        auto [points_xy, kappaVector] = handler.sampleSingleSpline(result->coeffs_x, result->coeffs_y, params);
