...
public:
  void setNumLayers(NodeMap& nodesPerLayer) {
      num_layers = static_cast<int>(nodesPerLayer.size());
  }
  ...
  IPairVector getChildList(const IPair &srcIdx)  {
    ...
    for (auto &value : adjLists[srcIdx])    {
  ...
  auto calcSplines(const MatrixXd &path,
                   double psi_s,
                   double psi_e,
  ...
  void pruneEdge(NodeMap &nodesPerLayer, const DVector& raceline_vx, YAML::Node &params)  {
